...
function redirectLoggedInUser() {
  ...
  if (role === "WORKER") {
    window.location.href = "../HTML/WorkerDashboard.html";
...
function changeServiceImages() {
  ...
  images.forEach((image) => {
    const service = image.dataset.service;

    const serviceImageList = serviceImages[service];
    ...
    setTimeout(() => {
      ...
      if (imageIndexes[service] >= serviceImageList.length) {
        imageIndexes[service] = 0;
      }
      image.src = serviceImageList[imageIndexes[service]];
      // Fade in
